...
class CH9121:
    ...
    gateway      = (192,168,0,43)     # gateway / Router of LAN
    subnet       = (255,255,255,0)    # subnet for LAN
    localip      = (192,168,0,81)     # local IP of CH9121 on LAN, for both Uart0 & Uart1 
    ...
    devicename   = b''                # devicename of CH9121 as seen on the network, replace "PicoEth9121" (maximum length 28 bytes)
    ...
    u0portauto   = YES                # local port number set randomly instead of static port, NO/YES
    ...
    u1targetip   = (192,168,0,157)    # IP of destination for UART1
    ...
    u1portauto   = YES                # local port number set randomly instead of static port, NO/YES
    ...
    def __init__(self, uarts = 1, config = False):  #uarts are the number of uarts in use: 1 or 2
        ...
        #Setup the uarts
        self.u1on = self.NO
        self.u0 = UART(0, baudrate=self.u0baud, tx=Pin(0), rx=Pin(1), timeout = 10, timeout_char = 10) #uart0 baud=115200, data bits=8, parity=None, stop bit=1
        if uarts > 1:
            self.u1on = self.YES
            self.u1 = UART(1, baudrate=self.u1baud, tx=Pin(4), rx=Pin(5), timeout = 10, timeout_char = 10) #uart1 baud=115200, data bits=8, parity=None, stop bit=1
    ...
    def __str__(self):
        lcdtd = ("TCP/S","TCP/C","UDP/S","UDP/C")
        
        lip     = f'{self.localip[0]}.{self.localip[1]}.{self.localip[2]}.{self.localip[3]}'
        u0tip   = f'{self.u0targetip[0]}.{self.u0targetip[1]}.{self.u0targetip[2]}.{self.u0targetip[3]}'
        u1tip   = f'{self.u1targetip[0]}.{self.u1targetip[1]}.{self.u1targetip[2]}.{self.u1targetip[3]}'
        u1      = f''
        if self.u1on == self.YES:
            u1 = f'\nEthernet UART1 {lcdtd[self.u1mode]} {lip} p{self.u1localport} > {u1tip} p{self.u1targetport}'
        return f'Ethernet UART0 {lcdtd[self.u0mode]} {lip} p{self.u0localport} > {u0tip} p{self.u0targetport}{u1}\n'
    ...
    def config(self):    
        ...
        cmd(b'\x34'+self.devicename) #CH9121 devicename as seen on the network, replace "PicoEth". (Optional) (maximum length 28 bytes)   
    ...
    def readsettings(self):
        ...
        if u1mode != chmode[0] and u0mode != chmode[1]:
        ...
        """CH9121 top line for a 20x4 LCD"""
    def lcdtop(self): 
        lcdtd = ("TCP/S","TCP/C","UDP/S","UDP/C")
        vect  = ">"
        #if self.u0mode == 0 or self.u0mode == 2:
            #vect  = "<"
        path  = f"{self.localip[3]} {vect} {self.u0targetip[3]}"
        if self.u1on == self.YES:
            path += f"/{self.u1targetip[3]}"
        space = " " * (20 - len(path) - 5)
        return (f"{path}{space}{lcdtd[self.u0mode]}")